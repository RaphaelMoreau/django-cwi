...
class ApplicationAddAdConfigForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.application_id:
            self.fields['country'].queryset = Country.objects.exclude(codeA2__in=ApplicationAdConfig.objects.filter(application_id=self.application_id).values_list('country'))
